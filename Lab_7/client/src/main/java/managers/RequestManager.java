...
public class RequestManager {
    ...
    public DragonType requestDragonType() {
        ...
        while (true){
            try {
                System.out.println("Категории: " + DragonType.nameList());
    ...
    public DragonCharacter requestDragonCharacter() {
        ...
        while (true){
            try {
                System.out.println("Категории: " + DragonCharacter.nameList());
    ...
    public Color requestColor() {
        ...
        while (true){
            try {
                System.out.println("Категории: " + Color.nameList());
                ...
                ConsoleManager.printError("Некорректный аргумент");
            }
        }
        return color;
    }
    ...
    /**
     * Метод запрашивает у пользователя авторизацию
     * @param argument
     * @return x - логическая переменная, являющаяся ответом на вопрос. true - auth, false - reg.
     */
    public boolean logIn(String argument) {
        String question = argument + "  auth/reg";
        String answer;
        boolean x;
        while (true) {
            try {
                System.out.println(question);
                answer = scanner.nextLine().trim();
                if (answer.equalsIgnoreCase("auth")) x = true;
                else if(answer.equalsIgnoreCase("reg")) x = false;
                else throw new WrongAnswerFormatException();
                break;
            } catch (WrongAnswerFormatException exc) {
                ConsoleManager.printError("Ответ должен быть 'auth' или 'reg'!");
            }
        }
        return x;
    }
     ...
     * @param argument
     * @return fetch - строка, содержащая логин, хэшированный пароль и пометку входа или авторизации
     */
    public String askPassword(boolean argument) {
        String question = "Введите логин и пароль через пробел";
        String key;
        String ans;
        if (argument) key = "1";
        else key = "0";
        while (true) {
            try {
                System.out.println(question);
                String[] answer = scanner.nextLine().trim().split(" ");
                if (answer.length>2) throw new WrongAnswerFormatException();
                for (String word : answer){
                    if (!word.equalsIgnoreCase("")){}
                    else throw new WrongAnswerFormatException();
                }
                String login = answer[0];
                String password;
                if (answer.length>1) password = org.apache.commons.codec.digest.DigestUtils.sha256Hex(answer[1]);
                else password = org.apache.commons.codec.digest.DigestUtils.sha256Hex("PASSWORD");
                ans = "auth"+"~~~"+password+"~/"+key+"&&&"+login;
                break;
            } catch (WrongAnswerFormatException exc) {
                ConsoleManager.printError("Нужно ввести только логин и пароль! Логин и пароль не должны включать пробелы");
            }
        }
        return ans;
    }
